"""
Test script for fine-tuned FlowLet model on the OpenBHB brain age prediction task.

Evaluates the model on the test split and saves metrics to disk.
...
Usage:
    # Basic test
    python scripts/test_brain_age.py \
        --checkpoint path/to/checkpoint.ckpt \
        --data_dir /path/to/openbhb_data \
        --metadata_file /path/to/train.tsv \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test fine-tuned FlowLet on OpenBHB brain age prediction"
    )
    ...
    parser.add_argument(
        ...
        help="Path to metadata TSV/CSV/parquet file",
    )
    ...
    parser.add_argument(
        ...
        help="Fraction of data to use as validation set (excluded from test)",
    )
    parser.add_argument(
...
def build_test_dataloader(
    ...
    num_workers: int,
    seed: int,
) -> Tuple[DataLoader, 'torch.utils.data.Subset']:
    """Build the test DataLoader using the same split as training."""
    ...
    print(f"Dataset split: total={n_total}, train={n_train}, val={n_val}, test={n_test}")
...
def compute_ssim_3d(
    ...
    """Compute mean SSIM over axial slices between two 3D volumes.
    ...
    try:
        ...
        # Simplified SSIM using means/variances (Wang et al. 2004)
...
def main():
    ...
    print("=" * 70)
    print("FlowLet - Brain Age Prediction Test (OpenBHB)")
    print("=" * 70)
    ...
    with open(summary_path, "w") as f:
        f.write("FlowLet - Brain Age Prediction Test Results (OpenBHB)\n")
        f.write("=" * 60 + "\n\n")
