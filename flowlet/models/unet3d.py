...
class UNet3DSmall(UNet3D):
    """
    Smaller 3D U-Net for memory-constrained scenarios.

    Reduces hidden dimensions and removes attention for efficiency.
    """
    ...
    def __init__(
        ...
        attention_levels: List[int] = [],  # No attention
        dropout: float = 0.0,
        groups: int = 8,
    ):
        super().__init__(
...
class UNet3DLarge(UNet3D):
    """
    Larger 3D U-Net for high-quality generation.

    Increases hidden dimensions and adds more residual blocks.
    """
    ...
    def __init__(
        ...
        attention_levels: List[int] = [2, 3, 4],
        dropout: float = 0.1,
        groups: int = 8,
    ):
        super().__init__(
